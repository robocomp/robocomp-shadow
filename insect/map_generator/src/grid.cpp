...
cv::Mat Grid::draw_map(const std::string &filename, int width, int height)
{
    std::vector<int> compression_params; // Stores the compression parameters
    compression_params.push_back(cv::IMWRITE_PXM_BINARY); // Set to PXM compression
    compression_params.push_back(0); // Set type of PXM in our case PGM

    int height_res = height / params.tile_size;
    int width_res = width / params.tile_size;
    cv::Mat img = cv::Mat::zeros(height_res + 1, width_res + 1, CV_8UC1);
    for (auto &&[k, v]: fmap)
    {
        // print cell
        cv::Scalar color;
        ...
        else if (v.cost == params.free_cost)
            color = cv::Scalar(255, 255, 255);
        else
            color = cv::Scalar(128, 128, 128);
            img.at<uchar>((k.second + height / 2)/ params.tile_size, (k.first + height / 2)/ params.tile_size) = color[0];
    }
    // invert image with respect to the x axis
    cv::flip(img, img, 0);
    // resize img x10 using internearest
    std::string file_name = filename + ".pgm";
    generate_yaml(filename);
    cv::imwrite(file_name, img, compression_params);
...
void Grid::generate_yaml(const std::string &filename)
{
    YAML::Node node;
    node["image"] = filename + ".pgm";
    node["resolution"] = params.tile_size;
    node["origin_x"] = 5000;
    node["origin_y"] = 5000;
    ...
    std::string file_name = filename + ".yaml";
    std::ofstream file(file_name);
    file << yamlString;
    file.close();

    std::cout << "YAML file saved as 'output.yaml'" << std::endl;
}
