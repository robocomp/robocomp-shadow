...
/** \mainpage RoboComp::lidar_odometry
 *
 * \section intro_sec Introduction
 *
 * The lidar_odometry component...
 *
 ...
 *
 * Just: "${PATH_TO_BINARY}/lidar_odometry --Ice.Config=${PATH_TO_CONFIG_FILE}"
 *
...
class lidar_odometry : public RoboComp::Application
{
public:
	lidar_odometry (QString prfx, bool startup_check) { prefix = prfx.toStdString(); this->startup_check_flag=startup_check; }
private:
	void initialize();
	std::string prefix;
	TuplePrx tprx;
...
void ::lidar_odometry::initialize()
{
...
int ::lidar_odometry::run(int argc, char* argv[])
{
#ifdef USE_QTGUI
	...
	try
	{
		...
		{
			// Server adapter creation and publication
			if (not GenericMonitor::configGetString(communicator(), prefix, "LidarOdometry.Endpoints", tmp, ""))
			{
				cout << "[" << PROGRAM_NAME << "]: Can't read configuration for proxy LidarOdometry";
			}
			Ice::ObjectAdapterPtr adapterLidarOdometry = communicator()->createObjectAdapterWithEndpoints("LidarOdometry", tmp);
			auto lidarodometry = std::make_shared<LidarOdometryI>(worker);
			adapterLidarOdometry->add(lidarodometry, Ice::stringToIdentity("lidarodometry"));
			adapterLidarOdometry->activate();
			cout << "[" << PROGRAM_NAME << "]: LidarOdometry adapter created in port " << tmp << endl;
		}
		catch (const IceStorm::TopicExists&){
			cout << "[" << PROGRAM_NAME << "]: ERROR creating or activating adapter for LidarOdometry\n";
		}
...
int main(int argc, char* argv[])
{
	...
	::lidar_odometry app(prefix, startup_check_flag);
