...
Ice.loadSlice("-I ./src/ --all ./src/WhisperStream.ice")
import RoboCompWhisperStream
...
class Control(list):
    def __init__(self, iterable=list()):
        super(Control, self).__init__(iterable)

    def append(self, item):
        assert isinstance(item, RoboCompGridPlanner.TControl)
        super(Control, self).append(item)

    def extend(self, iterable):
        for item in iterable:
            assert isinstance(item, RoboCompGridPlanner.TControl)
        super(Control, self).extend(iterable)

    def insert(self, index, item):
        assert isinstance(item, RoboCompGridPlanner.TControl)
        super(Control, self).insert(index, item)

setattr(RoboCompGridPlanner, "Control", Control)
class TObjects(list):
    def __init__(self, iterable=list()):
...
class Requires:
    def __init__(self, ice_connector):
        ...
        self.GridPlanner = self.create_proxy("GridPlannerProxy", RoboCompGridPlanner.GridPlannerPrx)
...
class Subscribes:
    def __init__(self, ice_connector, topic_manager, default_handler):
        ...
        self.WhisperStream = self.create_adapter("WhisperStreamTopic", whisperstreamI.WhisperStreamI(default_handler))
