...
class Dynamic_Window
{
    public:
        ...
        std::tuple<float, float, float> update(const Eigen::Vector3f &target_r, const std::vector<Eigen::Vector2f> &ldata, // x,y
                                               ...
                                               float current_side = 0.f,
                                               float current_rot = 0.f,
                                               const std::vector<std::tuple<float, float, bool>> &bumper = std::vector<std::tuple<float, float, bool>>(),
    ...
    private:
        ...
        std::vector<Dynamic_Window::Result> compute_predictions_holonomic(float current_adv,  float current_side,  float current_rot,
                                                                          const QPolygonF &laser_poly, const std::vector<std::tuple<float, float, bool>> &bumper);
        bool point_reachable_by_robot(const Result &point, const QPolygonF &laser_poly, const std::vector<std::tuple<float, float, bool>> &bumper);
        ...
        struct Constants
        {
            ...
            const float max_advance_speed = 500.f;
            const float min_advance_speed = -500.f;
            const float max_side_speed = 300.f;
            const float min_side_speed = -300.f;
            const float max_rotation_velociy = 2;
            const float adv_accel = 300; // mm/sg2
            const float rot_accel = 1.f; // rads/sg2
            const float side_accel =  200;  // mm/sg2
            const float adv_step = 50;
            const float side_step = 50;
            const float rot_step = 0.2;
            float max_reachable_adv_speed =  adv_accel * time_ahead;
            float min_reachable_adv_speed =  -adv_accel * time_ahead;
            float max_reachable_rot_speed =  rot_accel * time_ahead;
            float min_reachable_rot_speed =  -rot_accel * time_ahead;
            float max_reachable_side_speed =  side_accel * time_ahead;
            float min_reachable_side_speed =  -side_accel * time_ahead;
        };
        Constants constants;
