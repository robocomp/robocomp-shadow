...
void SpecificWorker::initialize(int period)
{
    ...
    else
    {
        ...
        try
        {
            ...
            std::cout << "FOCAL X: " << cam_intr.fx << std::endl;
...
void SpecificWorker::compute()
{
    ...
    if (not depth or not color)
        return;
    cv::Mat frame_color(cv::Size(color.get_width(), color.get_height()), CV_8UC3, (void*)color.get_data(), cv::Mat::AUTO_STEP);
    cv::Mat frame_depth(cv::Size(depth.get_width(), depth.get_height()), CV_16UC1, (void*)depth.get_data(), cv::Mat::AUTO_STEP);
    cv::rotate(frame_color, frame_color, cv::ROTATE_90_COUNTERCLOCKWISE);
    cv::rotate(frame_depth, frame_depth, cv::ROTATE_90_COUNTERCLOCKWISE);
    ...
    if (display_rgb)
    {
        ...
        cv::imshow("RGB image", frame_color);
        cv::waitKey(1);
    }
    ...
    if (display_depth)
    {
        ...
        cv::Mat display_frame_depth(cv::Size(w, h), CV_8UC3, (void*)depth_color.get_data(), cv::Mat::AUTO_STEP);
        cv::rotate(display_frame_depth, display_frame_depth, cv::ROTATE_90_COUNTERCLOCKWISE);
        cv::imshow("DEPTH image", display_frame_depth);
        cv::waitKey(1);
    }
    rgb_frame_write = std::make_tuple(frame_color, now);
    depth_frame_write = std::make_tuple(frame_depth, now);
    swap_mutex.lock();
...
RoboCompCameraRGBDSimple::TRGBD SpecificWorker::CameraRGBDSimple_getAll(std::string camera)
{
    ...
    if(not ready_to_go) return rgbd;
    const std::lock_guard<std::mutex> lg(swap_mutex);
    auto &[rgb, timestamp] = rgb_frame_read;
    // auto rgb = rgb_.as<rs2::video_frame>();
    int width = rgb.cols;
    int height = rgb.rows;
    int rgb_bpp = rgb.channels();
    ...
    {
        uint8_t* byte_frame = reinterpret_cast<uint8_t*>(rgb.data);
    ...
    auto &[frame, dtimestamp] = depth_frame_read;
    ...
    else
    {
        ...
        const uint16_t* byte_frame = reinterpret_cast<const uint16_t*>(frame.data);
        float real_depth;
        std::size_t k = 0;
...
RoboCompCameraRGBDSimple::TDepth SpecificWorker::CameraRGBDSimple_getDepth(std::string camera)
{
    ...
    const std::lock_guard<std::mutex> lg(swap_mutex);
    auto &[frame, timestamp] = depth_frame_read;
    // auto frame = frame_.as<rs2::video_frame>();
