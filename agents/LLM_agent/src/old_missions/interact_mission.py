...
class InteractMission:
    def __init__(self, graph, agent_id, mission):
        try:
    ...
    def monitor(self):
        ...
        if self.send_prompt_flag:
            ...
            if valid:
                ...
                response["timestamp"] = response["timestamp"] = round(time.time() - self.mission_start_time, 2)
    ...
    def generate_state_data(self):
        ...
        if self.person_id:
            ...
            asr_words = f"La persona ha dicho: {self.ASR_words.popleft()}" if len(self.ASR_words) > 0 else ""
