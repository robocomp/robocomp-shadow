...
class LongTermGraph:
    def __init__(self, file_name):
        self.node_types_to_avoid = ["wall", "corner", "room"]
        try:
            self.g = self.read_graph(file_name, directed=False)
        ...
        #
        # self.draw_graph(only_rooms=False)
        self.check_connected_doors()

    def check_connected_doors(self):
        doors = self.g.vs.select(type_eq="door")
        # Check connectivity of doors between doors
        for door in doors:
            # Get the neighbors of the door node
            neighbors = self.g.neighbors(door, mode='all')
            # Check if there is a door in the neighbors
            connected_doors = [self.g.vs[n] for n in neighbors if self.g.vs[n]["type"] == "door"]
            if len(connected_doors) < 2:
                print(f"Door {door['name']} is not connected to any other door.")

            else:
                print(f"Door {door['name']} is connected to {len(connected_doors)} other doors.")
                # Print other doors connected to this door
                for connected_door in connected_doors:
                    print(f"Connected door: {connected_door['name']}")

    def get_igraph_rooms(self):
        """ Returns a list of room names in the graph"""
        rooms = self.g.vs.select(type_eq="room")
        return [room["name"] for room in rooms]

    def get_igraph_elements(self):
        """ Returns a list of room names in the graph"""
        elements = self.g.vs.select(type_eq="robot_charger")
        return [element["name"] for element in elements]

    def get_igraph_elements_given_room_name(self, room_name: str):
        """
        Given a room name, return all child nodes of a specific type.

        Parameters
        ----------
        room_name : str
            The name of the room (vertex attribute "name").
        child_type : str
            The type of child nodes to retrieve (vertex attribute "type").

        Returns
        -------
        list
            A list of child vertices (igraph.Vertex objects).
        """

        # Find the room vertex by name
        room_vertices = self.g.vs.select(name=room_name)
        if len(room_vertices) == 0:
            raise ValueError(f"No room found with name: {room_name}")

        room_vertex = room_vertices[0]

        # Get neighbors (children) of the room
        neighbors = self.g.neighbors(room_vertex, mode="out")

        # Filter neighbors by attribute type
        children = [
            self.g.vs[n]
            for n in neighbors
            if self.g.vs[n]["type"] not in self.node_types_to_avoid
        ]

        return children

    def find_path_between_nodes(self, origin_name: str, target_name: str):
        print("Finding path between rooms:", origin_name, "and", target_name)
        # Encuentra los índices de los nodos "room" por nombre
        origin = self.g.vs.find(name=origin_name).index
        target = self.g.vs.find(name=target_name).index

        if self.g.shortest_paths(origin, target)[0][0] == float('inf'):
            print("No hay camino entre los nodos.")

        # Busca el camino más corto entre las dos habitaciones
        path = self.g.get_shortest_paths(origin, to=target, output="vpath")[0]
        print("Path found:", path)
        # Extrae los nodos en ese camino
        node_path = self.g.vs[path]

        # Puedes mostrar nombre y tipo para cada nodo del camino
        # detailed_path = [(v["type"], v["name"]) for v in node_path]
        detailed_path = [v["name"] for v in node_path if not v["type"] in self.node_types_to_avoid]
        # Remove even list positions (doors from the entering room FOV)
        clean_path = self.clean_path(detailed_path)
    ...
    def clean_path(self, path):
        ...
        for i, node in enumerate(path):
            ...
            if node.startswith("door"):
                # Si el siguiente también es door, nos quedamos con este y saltamos el siguiente
                if i + 1 < len(path) and path[i + 1].startswith("door"):
                    cleaned.append(node)
                    skip_next = True
                else:
                    cleaned.append(node)
    ...
    def read_graph(self, file_name, directed=True):
        ...
        g_directed = ig.Graph(directed=directed)
