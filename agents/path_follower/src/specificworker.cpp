...
void SpecificWorker::compute()
{
    ...
    if(robot_is_active = true)
    {
        if (auto path_o = path_buffer.try_get(); path_o.has_value()) // NEW PATH!
        {
            ...
            if(is_cyclic.load())
                saved_path = path;
            if(!isnan(path.back().x()) && !isnan(path.back().y()))
...
//    float euc_dist_to_target = (robot_pose - target).norm();
    float euc_dist_to_target = dist_along_path(path);
...
// WITH PARAMETRICED LINE
    if(robot_pose.x() != 0 and robot_pose.y() != 0 and path.size() > 2)
    {
        Eigen::ParametrizedLine<float, 2> line = Eigen::ParametrizedLine<float, 2>::Through(path[1], path[0]);
