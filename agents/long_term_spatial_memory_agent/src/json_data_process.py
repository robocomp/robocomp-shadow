...
# Open the json file
with open(f"/home/robocomp/robocomp/components/proceduralRoomGeneration/generatedRooms/20/apartmentData.json") as f:
    gt_data = json.load(f)
    # Get first room center
    ...
    print(gt_data['rooms'])
# Considering files which name starts with "generated_data", iterate over them
for file in os.listdir(f"/home/robocomp/robocomp/components/proceduralRoomGeneration/generatedRooms/20"):
    if file.startswith("generated_data"):
        # Open the json file
        with open(f"/home/robocomp/robocomp/components/proceduralRoomGeneration/generatedRooms/20/{file}") as f:
            data = json.load(f)
            ...
            for i in range(len(gt_data['rooms'])):
                for j in range(len(data['rooms'])):
                    cost_matrix[i][j] = math.sqrt(pow(gt_data['rooms'][i]['center'][0] - data['rooms'][j]['center'][0], 2) + pow(gt_data['rooms'][i]['center'][1] - data['rooms'][j]['center'][1], 2))
            ...
            for i in range(len(row_ind)):
                ...
                error_x = gt_data['rooms'][row_ind[i]]['center'][0] - data['rooms'][col_ind[i]]['center'][0]
                error_y = gt_data['rooms'][row_ind[i]]['center'][1] - data['rooms'][col_ind[i]]['center'][1]
            ...
            for i in range(len(gt_data['doors'])):
                for j in range(len(data['doors'])):
                    cost_matrix[i][j] = math.sqrt(pow(gt_data['doors'][i]['center'][0] - data['doors'][j]['center'][0], 2) + pow(gt_data['doors'][i]['center'][1] - data['doors'][j]['center'][1], 2))
            row_ind, col_ind = linear_sum_assignment(cost_matrix)
            ...
            for i in range(len(row_ind)):
                ...
                error_x = gt_data['doors'][row_ind[i]]['center'][0] - data['doors'][col_ind[i]]['center'][0]
                error_y = gt_data['doors'][row_ind[i]]['center'][1] - data['doors'][col_ind[i]]['center'][1]
            ...
            min_x = min([room['center'][0] - room['x'] / 2 for room in data['rooms']])
            max_x = max([room['center'][0] + room['x'] / 2 for room in data['rooms']])
            min_y = min([room['center'][1] - room['y'] / 2 for room in data['rooms']])
            max_y = max([room['center'][1] + room['y'] / 2 for room in data['rooms']])
            for room in data['rooms']:
                ax.add_patch(
                    patches.Rectangle((room['center'][0] - room['x'] / 2, room['center'][1] - room['y'] / 2), room['x'],
                                      room['y'], fill=False, color="red"))
                ax.add_patch(patches.Circle((room['center'][0], room['center'][1]), 100, fill=False, color="red"))
            for door in data['doors']:
                ax.add_patch(patches.Circle((door['center'][0], door['center'][1]), door['width'] / 2, fill=False, color="red"))
            # Adapt the axis to the rooms and doors
            plt.xlim(min_x - 1000, max_x + 1000)
            plt.ylim(min_y - 1000, max_y + 1000)
            plt.show()