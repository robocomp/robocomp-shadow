...
class LongTermGraph:
    """
    Provides methods for computing and visualizing a metric map of a long-term
    graph, including computing the shortest path between two rooms, transforming
    a room to another location, and drawing the metric map.

    Attributes:
        g (Graph): Used to represent the graph structure of a building's floor
            plan, with nodes representing rooms and edges representing doorways
            between them.
        read_graph (instance): Used to read a graph from a file specified by the
            user.
        fig (matplotlibFigure): Used to store the figure object that represents
            the graph.
        ax (Axes): Used to create the figure for visualizing the metric map.

    """
    def __init__(self, file_name):
        """
        Of the `LongTermGraph` class reads a graph from a file, creates a visualization
        of it using Python's Matplotlib library, and displays the graph's summary
        information.

        Args:
            file_name (str): Used to specify the file name for reading the graph
                data from.
