...
void SpecificWorker::compute()
{
    ...
    {
        laser_poly = std::get<0>(ldata.value());
        laser_cart = std::get<1>(ldata.value());
    ...
    if(path.size() > 0) // continue operation on current path
    {
        auto nose_3d = inner_eigen->transform(grid_type_name, Mat::Vector3d(0, 360, 0), robot_name).value();
        auto current_robot_nose = QPointF(nose_3d.x(), nose_3d.y());
        auto current_robot_polygon = get_robot_polygon();  //in GRID coordinates. Think of a transform_multi
        qInfo() << "PREV PATH";
                                    ...
                                    const QPointF &current_robot_nose)
{
    ...
    // while( total_energy_ratio > constants.max_total_energy_ratio or iterations++ < constants.max_free_energy_iterations)
    {
        ...
        for (auto &&[i, group] : iter::enumerate(iter::sliding_window(path, 3)))
        {
            ...
            qInfo() << "center_min" << center_min;

            // compute gradient as [ pD / dx, pD / dy ]
            QVector2D grad((px_min - mx_min) / constants.delta, (py_min - my_min) / constants.delta);
            if( center_min < constants.max_distance_range)
            {

                // e_force = grad * (constants.max_distance_range - center_min) * constants.KE;
                e_force = grad * constants.KE;
                qInfo() << "grad" << grad;
                qInfo() << "e_force" << e_force;
            ...
            qInfo() << "p" << p.x() << p.y();
            qInfo() << "total" << total.x() << total.y();
            qInfo() << "temp_p" << temp_p.x() << temp_p.y();
...
void SpecificWorker::clean_points(std::vector<QPointF> &path, const QPolygonF &laser_poly,  const QPolygonF &current_robot_polygon)
{
    ...
    for (const auto &group : iter::sliding_window(path, 2))
    {
        ...
        // qInfo() << __FUNCTION__ << " dist <" << dist << 0.5 * constants.road_step_separation;
    ...
    // qInfo() << __FUNCTION__ << "Removed: " << points_to_remove.size();
...
void SpecificWorker::add_points(std::vector<QPointF> &path, const QPolygonF &laser_poly, const QPolygonF &current_robot_polygon)
{
    ...
    for (auto &&[k, group] : iter::enumerate(iter::sliding_window(path, 2)))
    {
        ...
        if ( k < path.size()-1) // do not insert just before last
            continue;
        float dist = QVector2D(p1 - p2).length();
        qInfo() << __FUNCTION__ << " dist >" << dist << constants.road_step_separation;
    ...
    qInfo() << __FUNCTION__ << "Added: " << points_to_insert.size();
...
{
    QPolygonF robotP;
    auto bLWorld = inner_eigen->transform(grid_type_name, robotBottomLeft, robot_name);
    auto bRWorld = inner_eigen->transform(grid_type_name, robotBottomRight, robot_name);
    auto tRWorld = inner_eigen->transform(grid_type_name, robotTopRight, robot_name);
    auto tLWorld = inner_eigen->transform(grid_type_name, robotTopLeft, robot_name);
    ...
    qInfo() << "ROBOT POON";
    qInfo() << bLWorld.value().x() << bLWorld.value().y();
    qInfo() << bRWorld.value().x() << bRWorld.value().y();
    qInfo() << tRWorld.value().x() << tRWorld.value().y();
    qInfo() << tLWorld.value().x() << tLWorld.value().y();
    return robotP;
}
bool SpecificWorker::is_visible(QPointF p, const QPolygonF &laser_poly)
{
    std::optional<Mat::Vector3d> pointInLaser = inner_eigen->transform(laser_name, Mat::Vector3d (p.x(),p.y(), 0), grid_type_name);
...
std::optional<std::tuple<QPolygonF, std::vector<QPointF>>> SpecificWorker::get_laser_data()
{
    ...
    {
        auto angles = G->get_attrib_by_name<laser_angles_att>(node_laser.value());
        auto dists = G->get_attrib_by_name<laser_dists_att>(node_laser.value());
        if (dists.has_value() and angles.has_value())
        {
            ...
            for (const auto &[angle, dist] : iter::zip(a, d))
            {
                ...
                Mat::Vector3d laserWorld = inner_eigen->transform(grid_type_name,Mat::Vector3d(x, y, 0), robot_name).value();
