...
/** \mainpage RoboComp::door_detector
 *
 * \section intro_sec Introduction
 *
 * The door_detector component...
 *
 ...
 *
 * Just: "${PATH_TO_BINARY}/door_detector --Ice.Config=${PATH_TO_CONFIG_FILE}"
 *
...
#define PROGRAM_NAME    "door_detector"
#define SERVER_FULL_NAME   "RoboComp door_detector::door_detector"
...
class door_detector : public Ice::Application
{
public:
	door_detector (QString configFile, QString prfx, bool startup_check) { 
...
Ice::InitializationData door_detector::getInitializationDataIce(){
...
int door_detector::run(int argc, char* argv[])
{
#ifdef USE_QTGUI
	...
	std::shared_ptr<IceStorm::TopicPrx> lidar3dpub_topic;
	Ice::ObjectPrxPtr lidar3dpub;
	...
	//Require code
	require<RoboCompG2Ooptimizer::G2OoptimizerPrx, RoboCompG2Ooptimizer::G2OoptimizerPrxPtr>(communicator(),
	                    configLoader.get<std::string>("Proxies.G2Ooptimizer"), "G2OoptimizerProxy", g2ooptimizer_proxy);
	...
	try
	{
		...
		//Subscribe code
		subscribe<Lidar3DPubI>(communicator(),
		                    topicManager, configLoader.get<std::string>("Endpoints.Lidar3DPubTopic"),
						    configLoader.get<std::string>("Endpoints.Lidar3DPubPrefix"), "Lidar3DPub", worker,  0,
						    lidar3dpub_topic, lidar3dpub, PROGRAM_NAME);
		...
		try
		{
			std::cout << "Unsubscribing topic: lidar3dpub " <<std::endl;
			lidar3dpub_topic->unsubscribe(lidar3dpub);
...
int main(int argc, char* argv[])
{
	...
	door_detector app(configFile, prefix, startup_check_flag);
