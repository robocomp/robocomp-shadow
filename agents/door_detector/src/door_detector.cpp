...
DoorDetector::Doors_list DoorDetector::get_doors(const DoorDetector::Peaks_list &peaks_list, const DoorDetector::Lines &lines, const std::vector<Eigen::Vector2f> &corners)
{
    ...
    for(const auto &[i, peaks] : peaks_list | iter::enumerate)
        for(auto &par : peaks | iter::combinations(2))
            ...
            if((lines[i][par[0]]-lines[i][par[1]]).norm() < consts.MAX_DOOR_WIDTH
                and (lines[i][par[0]]-lines[i][par[1]]).norm() > consts.MIN_DOOR_WIDTH)
            // Conditions for being over wall and not lidar points between peaks
...
void DoorDetector::draw_doors(const Doors &doors, const Door &door_target, QGraphicsScene *scene, QColor color)
{
    ...
    {
        // print doors data
        qInfo() << "Door p0" << d.p0.x() << d.p0.y() << "p1" << d.p1.x() << d.p1.y() << "idx0" << d.idx_in_peaks_0 << "idx1" << d.idx_in_peaks_1;
//        if(d == door_target)
//        {
...
//        else
        ...
        borrar.push_back(point);
        auto line = scene->addLine(d.p0.x(), d.p0.y(), d.p1.x(), d.p1.y(), QPen(target_color, 100));
        borrar.push_back(line);
    }
}
// Function for drawing peaks
void DoorDetector::draw_peaks(const Peaks_list &peaks_list, const Lines &lines, QGraphicsScene *scene)
{
    static std::vector<QGraphicsItem *> borrar;
    for (auto &b: borrar)
    {
    ...
    for(const auto &[i, peaks] : peaks_list | iter::enumerate)
        ...
        {
            auto point = scene->addRect(-100, -100, 200, 200, QPen(QColor("red")), QBrush(QColor("red")));
            point->setPos(lines[i][p].x(), lines[i][p].y());
            borrar.push_back(point);
        }
}
