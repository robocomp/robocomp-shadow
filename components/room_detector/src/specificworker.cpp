...
void SpecificWorker::initialize()
{
    ...
    else
    {
        ...
        connect(this, SIGNAL(createFridgeTransform()), viewer3D, SLOT(createFridgeTransform()));
        connect(this, SIGNAL(updateFridgeTransform()), viewer3D, SLOT(updateFridgeTransform()));
        ...
        // reset optimization. Connect pushButton_reset to reset the optimization
        connect(pushButton_reset, &QPushButton::clicked, [this]()
            { if (reset_optimiser) reset_optimiser = false; else reset_optimiser = true;});
        ...
        connect(slider_adj_cost, &QSlider::valueChanged, [this](auto v){ lcdNumber_adj->display((static_cast<float>(v/params.div_value_slider)));});
        slider_adj_cost->setValue(params.div_value_slider * rc::ActionableThing::params.ADJ_SIGMA);
        connect(slider_alignment_cost, &QSlider::valueChanged, [this](auto v){ lcdNumber_alignment->display((static_cast<float>(v/params.div_value_slider)));});
        slider_alignment_cost->setValue(params.div_value_slider * rc::ActionableThing::params.ALIGNMENT_SIGMA);

        // OJO: mismo valor para todas las instancias de ActionableThing
...
//                return false;
            ...
            for(const auto &l : lines)
                if(l.distance((best_actionable->get_robot_pose() * p).head(2)) < MD)
                    return false;
            return true;
        });
    ...
    std::ranges::transform(room_filtered_points, std::back_inserter(room_filtered_points_in_room_frame), [best_actionable](auto &p)
        {
          auto t_pt =  best_actionable->get_robot_pose() * p;
    ...
    // cluster points with dbscan and filter out the biggest cluster, for now. The come sorted by size
    const auto clusters = rc::dbscan_get_point_clusters(thing_filtered_points, 0.1, 10); //meters
    if(clusters.empty())
    { qWarning() << __FUNCTION__ << "No clusters found for thing_filtered_points" ; return;}
    thing_filtered_points = clusters.begin()->second;

    // process the actionables
    for (const auto &[i, act]: thing_actionables | iter::enumerate)
    {
        thing_filtered_points = act.project(thing_filtered_points, *best_actionable, this->reset_optimiser, &viewer_global->scene);
        this->reset_optimiser = false;  // reset button
        emit updateFridgeTransform();  // signal a change in the fridge. Should depend on being real changes.
        plot_free_energy(act.get_error(), act.get_traza()*params.TRACE_SCALING_FACTOR);
        lcdNumber_cx_current->display(act.get_fridge().x());
        lcdNumber_cy_current->display(act.get_fridge().y());
        lcdNumber_angle_current->display(act.get_fridge().z());
        lcdNumber_width_current->display(act.get_fridge()(3));
        lcdNumber_depth_current->display(act.get_fridge()(4));
    }
    if(thing_actionables.empty() and thing_filtered_points.size() > 50)
    {
        if(rc::ActionableThing thing(inner_model); thing.initialize( *best_actionable,
                                                                     thing_filtered_points,
                                                                     rc::ActionableThing::Params{.POINTS_SIGMA=slider_points_cost->value()/100.f,
                                                                         .ADJ_SIGMA=slider_adj_cost->value()/100.f,
                                                                         .ALIGNMENT_SIGMA=slider_alignment_cost->value()/100.f,
                                                                         .PRIOR_CX_SIGMA=slider_cx_prior->value()/100.f,
                                                                         .PRIOR_CY_SIGMA=slider_cy_prior->value()/100.f,
                                                                         .PRIOR_ALPHA_SIGMA=slider_alpha_prior->value()/100.f,
                                                                         .PRIOR_WIDTH_SIGMA=slider_width_prior->value()/100.f,
                                                                         .PRIOR_DEPTH_SIGMA=slider_depth_prior->value()/100.f},
                                                                     &viewer_global->scene))
        {
            // if a reasonable low error is reached, add the new actionable
            thing_actionables.emplace_back(thing);
            emit createFridgeTransform();
        }
    }
    ...
    try
    {
        ...
        for(const auto &a: ldata.points)
        {
            if(a.z > 1300 and a.distance2d > 200)
                p_filter.emplace_back(a.x, a.y, 1.0);
        }
