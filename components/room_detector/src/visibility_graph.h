...
namespace rc
{
    ...
    private:
        ...
        std::vector<std::vector<VisibilityGraph::Point>>
        dijkstra_three_best_paths(const std::map<Point, std::vector<std::pair<Point, double>>> &graph, const Point &start, const Point &goal);

        double frechet_distance(const std::vector<VisibilityGraph::Point> &path1, const std::vector<VisibilityGraph::Point> &path2);
    };
}
