...
namespace rc
{
    casadi::Opti MPC::initialize_differential(const int N)
    {
        ...
        generic_options["print_time"] = false;  // Disable printing of solver times
        //specific_options["ipopt.print_level"] = 0;  // Set IPOPT verbosity to 0
        //specific_options["ipopt.sb"] = "yes";                  // Suppress IPOPT banner
        //specific_options["ipopt.print_user_options"] = "no";   // Suppress user option printing
        ...
        //double dt = consts.time_interval;   // timer interval in secs
        double dt = 0.2;   // timer interval in secs
    ...
    casadi::Opti MPC::initialize_omni(const int N)
    {
        ...
        specific_options["acceptable_obj_change_tol"] = 1e-6;
        generic_options["verbose"] = false;  // Minimize verbosity
        specific_options["print_time"] = false;  // Disable printing of solver times
        specific_options["ipopt.print_level"] = 0;  // Set IPOPT verbosity to 0
        specific_options["ipopt.sb"] = "yes";                  // Suppress IPOPT banner
        specific_options["ipopt.print_user_options"] = "no";   // Suppress user option printing
        opti.solver("ipopt", generic_options, specific_options);
        ...
        for(const auto k : iter::range(N-1))
        {
            ...
            //auto x_next = state(all, k) + dt * integrate(state(all,k), control(all,k));
            //opti.subject_to(opti.bounded(-3.19, acc, 3.19));
            opti.subject_to(opti.bounded(-0.4, acc, 0.4));
            ...
            auto ang_acc = (w2-w1)/dt;
            opti.subject_to(opti.bounded(consts.min_rotation_acc, ang_acc, consts.max_rotation_acc));
        }
        ...
        // Tie first position to robot origin
    ...
    MPC::update(const std::vector<Eigen::Vector2f> &path_robot, const std::vector<Eigen::Vector2f> &obstacles)
    {
        ...
        // minimize distance to each element of path
        ...
        for (auto k: iter::range(consts.num_steps))
            sum_dist_target += casadi::MX::sumsqr(pos(all, k) - e2v(target_robot.cast<double>())) - consts.min_dist_to_target;

        // add all terms to the cost function
        opti_local.minimize( sum_dist_path +
                             sum_angle * 8 +
                             sum_dist_target * 0.05 +
                             5.0/(sum_dist_obstacle+0.0001)
                             + sum_rot);
