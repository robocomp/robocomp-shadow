...
Ice.loadSlice("-I ./src/ --all ./src/BulletSim.ice")
import RoboCompBulletSim
...
class TPath(list):
    ...
    def append(self, item):
        assert isinstance(item, RoboCompGridder.TPoint)
        super(TPath, self).append(item)

    def extend(self, iterable):
        for item in iterable:
            assert isinstance(item, RoboCompGridder.TPoint)
        super(TPath, self).extend(iterable)

    def insert(self, index, item):
        assert isinstance(item, RoboCompGridder.TPoint)
        super(TPath, self).insert(index, item)

setattr(RoboCompGridder, "TPath", TPath)
class TPaths(list):
    def __init__(self, iterable=list()):
        super(TPaths, self).__init__(iterable)

    def append(self, item):
        assert isinstance(item, RoboCompGridder.TPath)
        super(TPaths, self).append(item)

    def extend(self, iterable):
        for item in iterable:
            assert isinstance(item, RoboCompGridder.TPath)
        super(TPaths, self).extend(iterable)

    def insert(self, index, item):
        assert isinstance(item, RoboCompGridder.TPath)
        super(TPaths, self).insert(index, item)

setattr(RoboCompGridder, "TPaths", TPaths)
...
class Implements:
    def __init__(self, ice_connector, default_handler):
        self.ice_connector = ice_connector
        self.bulletsim = self.create_adapter("BulletSim", bulletsimI.BulletSimI(default_handler))
