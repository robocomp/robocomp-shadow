                                          ...
                                          const Eigen::Matrix<float, 3, 3, Eigen::RowMajor>& robot_pose_uncertainty)
{
    ...
    // Assuming RoomModel has methods to get half-width and half-height
    // Adjust these calls based on your actual RoomModel API
    ...
    const Eigen::Vector3d room_uncertainty(0.1, 0.1, 0.05);
    graph_.initializeRoom(room_half_width_, room_half_depth_, room_uncertainty);
    
    // Add initial robot pose
    const gtsam::Pose2 robot_pose = toPose2(robot_pose_in_room);
    const Eigen::Vector3d pose_unc(robot_pose_uncertainty(0,0),
                                  robot_pose_uncertainty(1,1),
                                   robot_pose_uncertainty(2,2));
    
    current_robot_pose_index_ = graph_.addRobotPose(robot_pose, pose_unc);
...
WallID ConsensusManager::determineWallAttachment(const Eigen::Vector3f& door_pose) const
{
    ...
    const float norm_theta = std::fmod(theta + M_PI, 2 * M_PI) - M_PI;
    ...
    const float min_score = std::min({score_n, score_s, score_e, score_w});
                                 ...
                                 const Eigen::Matrix<float, 3, 3, Eigen::RowMajor> &detection_uncertainty)
{
    ...
    // Add to graph
    const gtsam::Pose2 pose = toPose2(door_pose);
    const Eigen::Vector3d unc(detection_uncertainty(0,0),
                       detection_uncertainty(1,1),
                        detection_uncertainty(2,2));
    
    const size_t door_index = graph_.addObject(pose, attached_wall, wall_offset, unc);
