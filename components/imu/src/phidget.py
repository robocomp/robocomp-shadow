...
class IMUSample:
    ...
    # reloj de pared en el instante de ADQUISICIÓN (tomado dentro de _on_data, hilo del
    # driver), no de publicación/lectura posterior -- evita reintroducir el bug de
    # "processing time vs. origin time" ya depurado en imu_fusion (ver
    # room_concept_rotation.md / project_imu_base_fusion). Consumidores que publiquen
    # esta muestra (ICE/DDS) deben usar este campo como timestamp, no time.time() propio.
    wall_time_s:  float
...
class PhidgetIMU:
    ...
    def _on_data(self, _, acceleration, angularRate, magneticField,
        ...
        """
        # Reloj de pared tomado AQUÍ, en el instante de adquisición (hilo del driver,
        # invocado justo al llegar el dato por USB) -- no en compute()/drain(), que puede
        # leerlo hasta un periodo de sondeo más tarde.
        wall_s = time.time()
