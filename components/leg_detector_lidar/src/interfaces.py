...
Ice.loadSlice("-I ./src/ --all ./src/LegDetector2DLidar.ice")
import RoboCompLegDetector2DLidar
...
class Legs(list):
    def __init__(self, iterable=list()):
        super(Legs, self).__init__(iterable)

    def append(self, item):
        assert isinstance(item, RoboCompLegDetector2DLidar.Leg)
        super(Legs, self).append(item)

    def extend(self, iterable):
        for item in iterable:
            assert isinstance(item, RoboCompLegDetector2DLidar.Leg)
        super(Legs, self).extend(iterable)

    def insert(self, index, item):
        assert isinstance(item, RoboCompLegDetector2DLidar.Leg)
        super(Legs, self).insert(index, item)

setattr(RoboCompLegDetector2DLidar, "Legs", Legs)

import legdetector2dlidarI
...
class Implements:
    def __init__(self, ice_connector, default_handler):
        self.ice_connector = ice_connector
        self.legdetector2dlidar = self.create_adapter("LegDetector2DLidar", legdetector2dlidarI.LegDetector2DLidarI(default_handler))
