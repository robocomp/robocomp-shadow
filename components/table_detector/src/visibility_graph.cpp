...
//                    auto v2 = face->vertex(i)->point();
    ...
    double VisibilityGraph::frechet_distance(const std::vector<Point>& path1, const std::vector<Point>& path2)
    {
        int m = path1.size();
        int n = path2.size();
        std::vector<std::vector<double>> dp(m, std::vector<double>(n, -1.0));

        auto euclidean_distance = [](const VisibilityGraph::Point& p1, const VisibilityGraph::Point& p2)
            { return std::sqrt(std::pow(p1.x - p2.x, 2) + std::pow(p1.y - p2.y, 2));};

        std::function<double(int, int)> recurse = [&](int i, int j) {
            if (dp[i][j] != -1.0)
                return dp[i][j];

            double dist = euclidean_distance(path1[i], path2[j]);

            if (i == 0 && j == 0)
                dp[i][j] = dist;
            else if (i == 0)
                dp[i][j] = std::max(recurse(i, j - 1), dist);
            else if (j == 0)
                dp[i][j] = std::max(recurse(i - 1, j), dist);
            else
                dp[i][j] = std::max(std::min({recurse(i - 1, j), recurse(i, j - 1), recurse(i - 1, j - 1)}), dist);
