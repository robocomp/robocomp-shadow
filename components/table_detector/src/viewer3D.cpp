...
void Viewer3D::createTableTransform()
    {
        //qInfo() << __FILE__ << __FUNCTION__;
        ...
        // --- Table: represented as a cube ---
        m_tableEntity = new Qt3DCore::QEntity(m_rootEntity);
        auto *tableMesh = new Qt3DExtras::QCuboidMesh();
        tableMesh->setXExtent(width);  // width
        tableMesh->setYExtent(height);  // height
        tableMesh->setZExtent(depth);  // depth
        m_tableEntity->addComponent(tableMesh);
        auto *tableMat = new Qt3DExtras::QPhongMaterial(m_tableEntity);
    ...
    void Viewer3D::updateTableTransform()
    {
        auto table_params = inner_model->table->means;
        const double x = table_params(0);
        const double y = table_params(1);
        ...
        r.first()->setXExtent(width);  // width
        r.first()->setZExtent(depth);  // depth
        r.first()->setYExtent(height);  // height
    }
    //////////////////////////////////////////////////////7
